...
class DynamicUnet(nn.Module):
    ...
    def __init__(self, num_classes, encoder = False, last=True):
        super().__init__()
        #self.encoder = encoder
        if encoder:
            #encoder = nn.Sequential(encoder)
            #encoder = models.__dict__[encoder](pretrained = True, num_classes = num_classes)
            #self.encoder = locals()[encoder](pretrained = True, num_classes = num_classes)
            #self.encoder = __dict__[encoder](pretrained = True, num_classes = num_classes)
            #self.encoder = encoder
            model = models.__dict__[encoder](pretrained = True, num_classes = num_classes)
            cut, cut_lr = model_meta[encoder]
            encoder = (list(model.children())[:cut] if cut else [model])
            encoder = nn.Sequential(*encoder)
            self.encoder = encoder
    ...
    def forward(self, x):
        ...
        if not hasattr(self, 'upmodel'):
            ...
            for idx in self.sfs_idxs[::-1]:
                ...
                print("upinc and xinc", up_in_c, x_in_c)
                print("x_copy", x_copy.size())
                print('self.sfs', len(self.sfs))
            ...
            self.final_conv = nn.Conv2d(final_in_c, self.num_classes, 1).type(dtype)
